...
export default {
  data () {
    return {
      ...
      option: {
        ...
        autoCropWidth: 170, // 默认生成截图框宽度  (默认:80%)
        autoCropHeight: 130, // 默认生成截图框高度  (默认:80%)
        fixedBox: true, // 固定截图框大小 不允许改变  (默认:false)
        fixed: true, // 是否开启截图框宽高固定比例  (默认:true)
        fixedNumber: [1, 0.764], // 截图框比例  (默认:[1:1])
        enlarge: 1
      },
      isDisabled: false,
      downImg: '#'
  ...
  watch: {
    imgFile: function (file) {
      this.imgFile = file;
      // if(this.imgFile.size > 32){

      //   this.option.outputSize = 32/this.imgFile.size
      // }
