...
import { getDepthAccountList, getSpecialList, editArticle, returnPostDetail } from '@/api/creator'
export default {
    name: "edit",
    components: {
        Cropper,
    ...
    methods: {
        ...
        getArticle(){
            ...
            returnPostDetail(data).then( res => {
                console.log(res)
                let data = res.data.list
                this.brief = data.brief
                this.title = data.title
                this.imgId = data.image_id
                this.userId = data.user_id
                this.editorContent = data.content
                this.editor.txt.html(data.content)
                
                let arr = data.post_special_id.split(',')
                arr.forEach( (item,index) => {
                    arr[index] =  Number(item)
                })
                this.postspecial = arr
                console.log(arr)
                this.loading = false
            }).catch( err => {
                this.loading = false
