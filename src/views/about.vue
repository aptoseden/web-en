<template>
  <div>
    ...
    <div class="pagebg ab"  style=" background: url(./static/images/blank.jpg) no-repeat;"></div>
    <div class="container">
      <h1 class="t_nav">
        <span>停下来，停在现在。让所有的情绪感受、想法，所有的自相矛盾，自我否定，都保留在现在。</span>
        <a href="https://demo.hyxdsoft.com/web9" class="n1">网站首页</a>
        <a href="/" class="n2">心理咨询</a>
      </h1>
      <div class="news_infos">
        ...
        <div class="message_infos" v-if="openComment == '1'">
          ...
          <div class="noComment" v-if="comments.length ==0">你的留言是我们改进最大的动力！</div>
        </div>
...
export default {
  ...
  mounted () {
    ...
    $(window).scroll(function () {
      ...
      if (docHeight == winHeight + winScrollHeight) {
        ...
        this.$http.get('./static/json/getCommentList.json').then((response) => {
          if (response.data.code == that.$ECode.SUCCESS) {
            that.comments = that.comments.concat(response.data.data.records)
  ...
  created () {
    ...
    this.$http.get('./static/json/getMe.json').then((response) => {
    ...
    setCommentAndAdmiration () {
      ...
      if (webConfigData.createTime) {
        this.openComment = webConfigData.openComment
          ...
          if (response.data.code == this.$ECode.SUCCESS) {
            webConfigData = response.data.data
            // 存储在Vuex中
